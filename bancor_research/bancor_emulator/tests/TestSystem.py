...
def execute(fileName):
    ...
    def verifyState(state):
        ...
        for token in reserveTokenValues + poolTokenValues + [vbnt]:
            for account in [user for user in token._balances.keys() if type(user) is str]:
                assertAlmostEqual(state[token.symbol()]['account_' + account], toValue(token, token.balanceOf(account)), 0, '0.000000000001')
        ...
        for token in [reserveToken for reserveToken in reserveTokenValues if reserveToken is not bnt]:
            ...
            assertAlmostEqual(state[token.symbol()]['pool_tknStakedBalance'   ], toValue(token, stakedBalance)                             , 0, '0.000000000001')
            assertAlmostEqual(state[token.symbol()]['pool_tknTradingLiquidity'], toValue(token, tradingLiquidity.baseTokenTradingLiquidity), 0, '0.000000000001')
            assertAlmostEqual(state[token.symbol()]['pool_bntTradingLiquidity'], toValue(bnt  , tradingLiquidity.bntTradingLiquidity)      , 0, '0.000000000001')
            assertAlmostEqual(state[token.symbol()]['pool_bntCurrentFunding'  ], toValue(bnt  , currentPoolFunding)                        , 0, '0.000000000001')
            assertAlmostEqual(state[token.symbol()]['pool_spotRate'           ], toRatio(spotRateN      , spotRateD      )                 , 0, '0.000000000001')
            assertAlmostEqual(state[token.symbol()]['pool_averageRate'        ], toRatio(averageRateN   , averageRateD   )                 , 0, '0.000000000001')
            assertAlmostEqual(state[token.symbol()]['pool_averageInverseRate' ], toRatio(averageInvRateN, averageInvRateD)                 , 0, '0.000000000001')

        # Iterate all reserve tokens
        for token in reserveTokenValues:
            assertAlmostEqual(state[token.symbol()]['contract_masterVault'], toValue(token, token.balanceOf(masterVault)), 0, '0.000000000001')
            assertAlmostEqual(state[token.symbol()]['contract_epVault'    ], toValue(token, token.balanceOf(epVault    )), 0, '0.000000000001')
    ...
    for n in range(len(flow['operations'])):
        ...
        verifyState(operation['expected'])
