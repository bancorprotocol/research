...
import bancor3_simulator
from bancor3_simulator.core.utils import get_logger


def print_info():
    """Prints information about the system, bancor3, and dependencies of bancor3."""
    logger = get_logger(__name__)
    logger.info("bancor3_simulator version: %s" % bancor3_simulator.__version__)
    logger.info(
        "bancor3_simulator installation directory: %s" % get_bancor3_simulator_root()
    )
    print_sys_info()
    print_deps()
...
def get_bancor3_simulator_root():
    """Gets location where bancor3 is installed.

    Returns:
        Location where bancor3 is installed.
    """
    return os.path.dirname(bancor3_simulator.__file__)
