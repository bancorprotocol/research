...
def execute(fileName, decimals = -1):
    ...
    for pair in [[item.astype('float64') for item in pair] for pair in zip(frames, frames[1:])]:
...
execute('BancorNetworkMassiveFinancialScenario1', 142 if mode else 8)
execute('BancorNetworkMassiveFinancialScenario2', 142 if mode else 8)
