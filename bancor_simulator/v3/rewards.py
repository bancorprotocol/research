...
from bancor_simulator.v3.state import State, get_flat_distribution_rate_per_second, get_half_life_seconds, \
    get_staked_balance, get_total_rewards, get_prev_token_amt_distributed, get_pooltoken_balance, \
    get_protocol_wallet_balance, get_distribution_type, get_autocompounding_start_time, \
    get_unix_timestamp, get_total_standard_rewards_staked, StandardProgramState, \
    get_standard_reward_tkn_name, get_user_wallet_tokens, get_standard_reward_per_token, get_standard_reward_start_time, \
    get_standard_reward_end_time, get_standard_reward_last_update_time, get_standard_reward_rate, \
    get_user_pending_standard_rewards, get_user_reward_per_token_paid, \
    get_standard_reward_providers, get_remaining_standard_rewards, get_user_pending_rewards_staked_balance, \
    get_unclaimed_rewards, get_pooltoken_name
...
def calc_flat_token_amt_to_distribute(
    ...
    """
    flat_distribution_rate_per_second = get_flat_distribution_rate_per_second(state, tkn_name)
...
def distribute_autocompounding_program(
        state: State, tkn_name: str, unix_timestamp: int, verbose: bool = True
) -> State:
    """
    Distribute autocompounding rewards for a given program.
    """
    assert (
            tkn_name in state.autocompounding_programs
    ...
    time_elapsed = Decimal(unix_timestamp) - Decimal(start_time)
    ...
    if verbose:
        print(
            f" staked_amt = {staked_amt}"
        )
        print(
            f" erc20contracts_bntkn = {erc20contracts_bntkn}"
        )
        print(
            f" program_wallet_bntkn = {program_wallet_bntkn}"
        )
        print(
            f" total_rewards = {total_rewards}"
        )
        print(
            f" prev_token_amt_distributed = {prev_token_amt_distributed}"
        )
        print(
            f" token_amt_to_distribute = {token_amt_to_distribute}"
        )
        print(
            f" pool_token_amt_to_burn = {pool_token_amt_to_burn}"
        )
        print(
            f" remaining_rewards = {remaining_rewards}"
        )
...
def create_standard_reward_program(
    ...
    if not unix_timestamp <= start_time and start_time < end_time:
    ...
    state.standard_programs[id] = StandardProgramState(
        id=id,
        tkn_name=tkn_name,
    ...
    state.set_standard_rewards_vault_balance(rewards_token, unclaimed_rewards + total_rewards)
    return state, id


def terminate_standard_program(state: State, id: int, unix_timestamp: int) -> State:
    """
    Terminates the standard program.
    """
    tkn_name, rewards_token = get_standard_reward_tkn_name(state, id)
    remaining_rewards = calc_standard_rewards_remaining(state, id, unix_timestamp)
    ...
    state.set_standard_program_end_time(id, unix_timestamp)
    return state


def snapshot_standard_rewards(state: State, id: int, unix_timestamp: int, user_name: str) -> State:
    """
    ...
    new_update_time = min(unix_timestamp, end_time)
    ...
    provider_staked_amount = get_user_pending_rewards_staked_balance(state, id, user_name)
    ...
    new_pending_rewards = calc_user_pending_standard_rewards(provider_pending_rewards, provider_staked_amount,
                                                             new_reward_per_token, reward_per_token_paid)
...
def calc_standard_reward_per_token(state: State, id: int, unix_timestamp: int) -> Decimal:
    """
    ...
    staking_end_time = min(unix_timestamp, end_time)
    staking_start_time = max(start_time, last_update_time)

    return reward_per_token + (staking_end_time - staking_start_time) * reward_rate / total_staked


def calc_user_pending_standard_rewards(provider_pending_rewards: Decimal, provider_staked_amount: Decimal,
                                       updated_reward_per_token: Decimal, reward_per_token_paid: Decimal) -> Decimal:
    """
    Calculates the standard rewards pending for a given user
    """
    return provider_pending_rewards + provider_staked_amount * (updated_reward_per_token - reward_per_token_paid)


def calc_standard_rewards_remaining(state: State, id: int, unix_timestamp: int) -> Decimal:
    """
    Calculates the standard rewards remaining for a given program id.
    """
    reward_rate = get_standard_reward_rate(state, id)
    end_time = get_standard_reward_end_time(state, id)
    return reward_rate * (end_time - unix_timestamp) if end_time > unix_timestamp else Decimal(0)


def join_standard_reward_program(state: State, user_name: str, id: int, pool_token_amount: Decimal,
                                 unix_timestamp: int) -> State:
    """
    Join a standard reward program with the given user and id.
    """
    tkn_name, rewards_token = get_standard_reward_tkn_name(state, id)
    assert tkn_name in get_user_wallet_tokens(state,
                                              user_name), f"User {user_name} does not have token {tkn_name} in wallet"

    state = snapshot_standard_rewards(state, id, unix_timestamp, user_name)
    state.decrease_user_balance(user_name, get_pooltoken_name(tkn_name), pool_token_amount)
...
def leave_standard_reward_program(state: State, user_name: str, id: int, pool_token_amount: Decimal,
                                  unix_timestamp: int) -> State:
    """
    Leave a standard reward program for the given user and program id.
    """
    tkn_name, rewards_token = get_standard_reward_tkn_name(state, id)
    assert user_name in get_standard_reward_providers(state, id), f"User {user_name} not in standard reward providers"
    ...
    state.increase_user_balance(user_name, get_pooltoken_name(rewards_token), pool_token_amount)
    state.remove_user_from_standard_reward_program(id, user_name)
    return state


def claim_standard_rewards(state: State,
                           user_name: str,
                           ids: list,
                           unix_timestamp: int,
                           is_vault_updated: bool = False) -> State:
    """
...
def distribute_standard_rewards(state: State, user_name: str, rewards_token: str, reward_amt: Decimal) -> State:
